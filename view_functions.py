...
class EdgePerturbation():
    ...
    def do_trans(self, data):
        ...
        # Improved handling of edge_index checking
        if not hasattr(data, 'edge_index') or data.edge_index.numel() == 0:
            # Handle the case where edge_index is empty or not present
            # print("Warning: Received a graph with no edges.")
            return data

        node_num = data.x.size(0)  # Assuming x is [num_nodes, num_node_features]

        # Ensure edge_index is a tensor with shape [2, num_edges]
        if data.edge_index.dim() == 2 and data.edge_index.size(0) == 2:
            _, edge_num = data.edge_index.size()
        else:
            # Handle unexpected edge_index shape
            print("Warning: edge_index does not have expected shape [2, num_edges].")
            return data
...
class Diffusion():
    ...
    def __init__(self, mode="heat", alpha=0.2, t=5, add_self_loop=True):
        self.mode = mode
        self.alpha = alpha
        self.t = t
